...
var foodmenuCollection *mongo.Collection = database.OpenCollection(database.Client, "menu")
...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//later use Aggregation in Pipeline
...
func CreateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := foodmenuCollection.FindOne(ctx, bson.M{"menu_id": food.Menu_id}).Decode(&menu)
		defer cancel()
		if err != nil {
		...
		if insertError != nil {
			msg := "food could not be inserted"
...
func UpdateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if food.Menu_id != nil {
			...
			defer cancel()
			if err != nil {
				msg := "could not find menu id to update the food item"
				c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
				return
			}
		...
		result, err := foodCollection.UpdateOne(
			...
			&opt)
		if err != nil {
			msg := "could not update the food item"
			c.JSON(http.StatusInternalServerError, msg)
			return
		}
		c.JSON(http.StatusOK, result)
